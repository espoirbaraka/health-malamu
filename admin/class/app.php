...
class App
{
    ...
    public function __construct($db_name = "health-malamu", $db_user = "root", $db_pass = "", $db_host = 'localhost')
    {
    ...
    public function substring($chaine, $nbre)
    {
        $strlen = strlen($chaine);
        if ($strlen > $nbre) {
            $replaces = substr($chaine, 0, $nbre) . str_repeat('.', 3);
            return $replaces;
        }else {
